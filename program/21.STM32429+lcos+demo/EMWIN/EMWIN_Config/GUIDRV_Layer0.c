    ...
    u32 a,r,g,b;
    {
      ...
      //
			...
			else
			{
				...
				g=(PixelIndex  >>8) & 0x000000ff;
				b=PixelIndex & 0x000000ff;
				FrameBuffer0[y][3*x] = a | r;
				FrameBuffer0[y][3*x+1] = a | g;
				FrameBuffer0[y][3*x+2] = a | b;
			}
    }
  ...
  unsigned int PixelIndex;
    ...
    u32 a,r,g,b;
    {
      ...
      //
			...
			{
				a=FrameBuffer0[y][3*x] & 0xff000000;
				r=FrameBuffer0[y][3*x] & 0x000000ff;
				g=FrameBuffer0[y][3*x+1] & 0x000000ff;
				b=FrameBuffer0[y][3*x+2] & 0x000000ff;
				PixelIndex = a | (r<<16) | (g<<8) | b;
			}
    }
    #if (LCD_MIRROR_X == 0) && (LCD_MIRROR_Y == 0) && (LCD_SWAP_XY == 0)
...
static void _SetOrg(GUI_DEVICE * pDevice, int x, int y) {
  ...
  LCD_X_DisplayDriver(pDevice->LayerIndex, LCD_X_SETORG, (void *)&Data);
	...
	DMA2D->BGMAR = (uint32_t)FrameBuffer0+(x+900*y)*4;
}
...
*/
const GUI_DEVICE_API GUIDRV_Layer0_API = {
  //
  // Data
  //
  DEVICE_CLASS_DRIVER,
