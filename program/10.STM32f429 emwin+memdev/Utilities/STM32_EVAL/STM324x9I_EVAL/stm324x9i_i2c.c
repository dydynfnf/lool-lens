...
void io_dir (uint32_t io, uint32_t dir) {
  if (io & IO_SDA) {
    GPIOH->MODER    &= ~(3 << 2 * 5);  /* SDA input */
    if (dir == IO_OUT) {
      GPIOH->MODER  |=  (1 << 2 * 5);  /* SDA output */
      GPIOH->OTYPER |=  (1 << 5);      /* Open-drain */
    }
  }
  if (io & IO_SCL) {
    GPIOH->MODER    &= ~(3 << 2 * 4);  /* SCL input */
    if (dir == IO_OUT) {
      GPIOH->MODER  |=  (1 << 2 * 4);  /* SCL output */
      GPIOH->OTYPER |=  (1 << 4);      /* Open-drain */
    }
  }
}
...
void io_pin (uint32_t io, uint32_t st) {
  if (io & IO_SDA) {
    if (st == IO_LOW) GPIOH->BSRRH |= (1 << 5);
    else              GPIOH->BSRRL |= (1 << 5);
  }
  if (io & IO_SCL) {
    if (st == IO_LOW) GPIOH->BSRRH |= (1 << 4);
    else              GPIOH->BSRRL |= (1 << 4);
  }
}
...
uint32_t io_state (uint32_t io) {
  if (io & IO_SDA) {
    return (GPIOH->IDR & (1 << 5)) ? (IO_HI) : (IO_LOW);
  }
  return   (GPIOH->IDR & (1 << 4)) ? (IO_HI) : (IO_LOW);
}
...
uint32_t I2C_Init (void) {
  ...
  /* Enable clock for I2C2, GPIOH and AFIO                                    */
  ...
  /* Set PH4, PH5 as I2C2 pins                                                */
  GPIOH->AFR[0]  &= ~0x00FF0000;
  GPIOH->AFR[0]  |=  0x00440000;        /* Alternate peripheral is I2C        */

//  GPIOH->AFR[0]  &= ~0x00F00000;
//  GPIOH->AFR[0]  |=  0x00400000;        /* Alternate peripheral is I2C        */

  GPIOH->MODER   &= ~0x00000F00;
  GPIOH->MODER   |=  0x00000A00;        /* Alternate function mode            */
  GPIOH->OTYPER  |=  0x00000030;        /* Pins are in open drain mode        */
  GPIOH->OSPEEDR &= ~0x00000F00;        /* Pins speed is 2MHz                 */
  GPIOH->PUPDR   &= ~0x00000F00;        /* No pull-up, no pull-down           */
  ...
  I2C2->CR2    =  0x001E;               /* Clock: 30MHz, interrupts disabled  */
  I2C2->CR1   &= ~I2C_CR1_PE;           /* Disable peripheral                 */
  I2C2->TRISE  =  0x1F;                 /* 1000ns / 33ns = 30 + 1             */
  I2C2->CCR    =  0x96;
  I2C2->CR1   |=  I2C_CR1_PE;           /* Enable peripheral                  */
  I2C2->CR1   |= 0x0400;                /* I2C mode, General call disabled,   */
                                        /* No start, no stop, ACK returned    */
  I2C2->OAR1   = (1UL << 14)      |     /* Ack 7bit address                   */
...
uint32_t I2C_Recovery (uint32_t err) {
  uint32_t i;
#if 1
  GPIOH->AFR[0]  &= ~0x00FF0000;        /* Set SDA and SCL as GPIO            */
  I2C2->CR1 |= I2C_CR1_SWRST;           /* Put I2C peripheral in reset mode   */
  ...
  I2C2->CR1 &= ~I2C_CR1_SWRST;          /* Dereset I2C peripheral             */
