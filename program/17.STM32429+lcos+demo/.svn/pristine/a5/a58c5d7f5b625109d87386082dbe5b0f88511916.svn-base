...
static void _SetPixelIndex(GUI_DEVICE * pDevice, int x, int y, int PixelIndex) {
    ...
    u32 a,r,g,b;
    {
      ...
      //
			if(MEM_EN)
			{
				//a=PixelIndex & 0xff000000;
				a=_COLOR & 0xff000000;
				r=(PixelIndex  >> 16) & 0x000000ff;
				g=(PixelIndex  >>8) & 0x000000ff;
				b=PixelIndex & 0x000000ff;
				MEM[y][3*x] = a | r;
				MEM[y][3*x+1] = a | g;
				MEM[y][3*x+2] = a | b;
			}
			else
			{
				...
				g=(PixelIndex  >>8) & 0x000000ff;
				b=PixelIndex & 0x000000ff;
				FrameBuffer1[y][3*x] = a | r;
				FrameBuffer1[y][3*x+1] = a | g;
				FrameBuffer1[y][3*x+2] = a | b;
			}
    }
  ...
  unsigned int PixelIndex;
    ...
    u32 a,r,g,b;
    {
      ...
      //
			if(MEM_EN)
			{
				a=MEM[y][3*x] & 0xff000000;
				r=MEM[y][3*x] & 0x000000ff;
				g=MEM[y][3*x+1] & 0x000000ff;
				b=MEM[y][3*x+2] & 0x000000ff;
				PixelIndex = a | (r<<16) | (g<<8) | b;
			}
			else
			{
				a=FrameBuffer1[y][3*x] & 0xff000000;
				r=FrameBuffer1[y][3*x] & 0x000000ff;
				g=FrameBuffer1[y][3*x+1] & 0x000000ff;
				b=FrameBuffer1[y][3*x+2] & 0x000000ff;
				PixelIndex = a | (r<<16) | (g<<8) | b;
			}
    }
    #if (LCD_MIRROR_X == 0) && (LCD_MIRROR_Y == 0) && (LCD_SWAP_XY == 0)
...
static void _SetOrg(GUI_DEVICE * pDevice, int x, int y) {
  ...
  LCD_X_DisplayDriver(pDevice->LayerIndex, LCD_X_SETORG, (void *)&Data);
	...
	DMA2D->FGMAR = (uint32_t)FrameBuffer1+(x+900*y)*4;
}
...
*/
const GUI_DEVICE_API GUIDRV_Layer1_API = {
  //
  // Data
  //
  DEVICE_CLASS_DRIVER,
