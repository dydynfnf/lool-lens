...
{  
	u8 byte=0;   
	W25QXX_CS=0;                            //使能器件   
	SPI5_ReadWriteByte(W25X_ReadStatusReg);    //发送读取状态寄存器命令    
...
{   
	W25QXX_CS=0;                            //使能器件   
	SPI5_ReadWriteByte(W25X_WriteStatusReg);   //发送写取状态寄存器命令    
...
void W25Qxx_Gpio_Init(void)
{
  ...
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_SPI5, ENABLE);//使能SPI1时钟
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_7|GPIO_Pin_8|GPIO_Pin_9;//PF7~9复用功能输出	
