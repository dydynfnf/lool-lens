...
static void _FillRect(GUI_DEVICE * pDevice, int x0, int y0, int x1, int y1) {
  ...
  if (GUI_pContext->DrawMode & LCD_DRAWMODE_XOR) {
    for (; y0 <= y1; y0++) {
	  ...
	  TFT_Fill(x0,y0,x1,y1,PixelIndex); 
  }
}
...
static void _DrawBitLine1BPP(GUI_DEVICE * pDevice, int x, int y, U8 const * p, int Diff, int xsize, const LCD_PIXELINDEX * pTrans) {
  ...
  case 0:
    do {
      LCD_WR_Data((*p & (0x80 >> Diff)) ? Index1 : Index0);//modified 
      if (++Diff == 8) {
        Diff = 0;
        p++;
      }
  ...
  case 0:
    if (pTrans) {
      do {
        ...
        LCD_WR_Data(PixelIndex);//modified 
        if (++CurrentPixel == 4) {
          CurrentPixel = 0;
          Pixels = *(++p);
        }
  ...
  case 0:
    if (pTrans) {
      for (; xsize > 0; xsize--, x++, p++) {
        Pixel = *p;
        LCD_WR_Data(*(pTrans + Pixel));//modified 
      }
    } else {
      for (; xsize > 0; xsize--, x++, p++) {
        LCD_WR_Data(*p);//modified 
      }
    }
    break;
  case LCD_DRAWMODE_TRANS:
