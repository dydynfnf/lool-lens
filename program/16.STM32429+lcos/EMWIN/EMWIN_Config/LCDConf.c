...
void LCD_X_Config(void) {
  ...
  //
  GUI_DEVICE_CreateAndLink(&GUIDRV_Layer1_API, COLOR_CONVERSION, 0, 0);
  //
  // Display driver configuration
  //
  if (LCD_GetSwapXY()) {
    LCD_SetSizeEx (0, YSIZE_PHYS, XSIZE_PHYS);
    LCD_SetVSizeEx(0, YSIZE_PHYS*2, XSIZE_PHYS);
  } else {
  ...
  //
  GUI_DEVICE_CreateAndLink(&GUIDRV_Layer2_API, COLOR_CONVERSION, 0, 1);
  //
  // Display driver configuration
  //
  if (LCD_GetSwapXY()) {
    LCD_SetSizeEx (1, YSIZE_PHYS, XSIZE_PHYS);
    LCD_SetVSizeEx(1, YSIZE_PHYS*2, XSIZE_PHYS);
  } else {
    LCD_SetSizeEx (1, XSIZE_PHYS, YSIZE_PHYS);
    LCD_SetVSizeEx(1, XSIZE_PHYS, YSIZE_PHYS*2);
  }
}
